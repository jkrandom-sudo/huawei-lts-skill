...
def _safe_user_path(path: Path, home: Path) -> Path:
    resolved = path.expanduser().resolve()
    resolved_home = home.expanduser().resolve()
    if resolved == resolved_home or resolved_home not in resolved.parents:
        raise ValueError(f"Refusing path outside user home: {resolved}")
    return resolved


def deploy_repo(source: Path, destination: Path, home: Path) -> None:
    destination = _safe_user_path(destination, home)
    marker_path = destination / MARKER_NAME
    if destination.exists() and any(destination.iterdir()) and not marker_path.exists():
        raise ValueError(f"Refusing to install into unowned non-empty directory: {destination}")
...
def install_skills(source: Path, home: Path, platforms: set[str]) -> None:
    ...
    for platform in platforms:
        target = home / PLATFORM_SKILL_DIRS[platform]
        backup = source / ".backups/skills" / platform
...
def uninstall(home: Path, destination: Path, platforms: set[str]) -> None:
    ...
    for platform in platforms:
        ...
        if target.exists():
            shutil.rmtree(target)
        backup = destination / ".backups/skills" / platform
        if backup.exists():
...
def main() -> int:
    ...
    parser.add_argument("action", choices=("install", "uninstall", "verify", "owned"))
    ...
    if args.action == "install":
        deploy_repo(args.source.resolve(), args.destination, args.home)
        install_skills(args.destination.resolve(), args.home, platforms)
    elif args.action == "uninstall":
        uninstall(args.home, args.destination, platforms)
    else:
        _, marker = _load_marker(args.destination, args.home)
